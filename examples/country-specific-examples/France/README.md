...
| `ISSUED_BY_PLATFORM` | 201 | Émise par la plateforme | Platform |
...
### Platform-Generated Status Examples

These examples illustrate CDAR lifecycle statuses generated by the Pagero / Thomson Reuters
platform (PA-E) following technical and semantic validation of the invoice. The sender of
both messages is the platform; the receiver is the seller (issuer of the original invoice).

#### 4. `PUF_France_200_Submitted.xml`

**Déposée — Invoice Response (Platform → Seller)**
Reference invoice: `FR-INV-2026-001` (TechDistrib France SARL → Entreprise Client France SA)

Demonstrates the `SUBMITTED` (200) status emitted by the platform once the invoice has been
accepted for further processing:

- Sender: Thomson Reuters (PA-E)
- Receiver: Seller — TechDistrib France SARL (SIREN 987654321)
- `puf:ReferencedDocumentInterchangeID` carries the platform interchange ID of the referenced invoice
- No `cac:Status` block — Déposée is an informational acknowledgement with no reason/action

> Déposée is one of the four statuses that must be reported to the Tax Authority (PPF).

---

#### 5. `PUF_France_213_Rejected.xml`

**Rejetée — Invoice Response (Platform → Seller)**
Reference invoice: `FR-INV-2026-002` (TechDistrib France SARL → Entreprise Client France SA)

Demonstrates the technical/semantic rejection (213) issued by the platform when validation
(schematron / business rules) fails on issuance:

- Sender: Thomson Reuters (PA-E)
- Receiver: Seller — TechDistrib France SARL (SIREN 987654321)
- Two `cac:Status` blocks, each with `listID=OPStatusReason` and code `REJ_SEMAN` (semantic rejection):
  - **BR-FR-09 / BT-29** — SIRET / SIREN coherence failure on the seller party
  - **BR-FR-05 / BT-22** — mandatory legal mentions (`#PMT#`, `#PMD#`, `#AAB#`) missing in BG-3 notes
- Each `StatusReason` describes the failure in French with the rule code, BT/BG reference,
  observed value, and XPath location of the offending element

> Rejetée is platform-generated and is one of the four statuses that must be reported to the
> Tax Authority. It differs from Refusée (210 / `RE` from buyer) — see the CDAR table above.
> Each validation failure should be reported as a separate `cac:Status` block so the seller
> can correct all errors in one pass.
...
##### 6. `PUF_France_UC2_PaymentReceived.xml`
