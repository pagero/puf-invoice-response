...
|`cbc:ReferenceID`
|**Response identifier** +
An identifier for the response itself, assigned by the party issuing it. +
Optional in PUF, but required by some target formats. In Turkey, UBL-TR makes
`cac:Response/cbc:ReferenceID` mandatory in an Uygulama Yanıtı.
...
|`cac:Status/cbc:Description`
|**Status note** +
A free-text note or comment about the status, providing additional information alongside any status reason or requested action.
...
<ApplicationResponse>
    ...
    <cac:DocumentResponse>
        <cac:Response>
            ...
            <cbc:ReferenceID>123456789</cbc:ReferenceID>
