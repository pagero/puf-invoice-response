...
`puf:ComplianceActivatedIndicator` indicates whether the document is processed within the French reform: `true` means Pagero (as PDP/PA) applies the stricter compliance checks. For lifecycle statuses this value is carried in the CDAR _cadre de facturation_ field (AFNOR MDT-2, the same field as the invoice's BT-23), which France repurposes to carry the document treatment type (rule BR-FR-CDV-CL-01): `REGULATED` activates compliance, while the other values (`NON_REGULATED`, `B2C`, `B2BINT`, `OUTOFSCOPE`) leave it `false`. `puf:ExternalDeliveryType` (`PROCESS_WITHOUT_DELIVERY` / `PROCESS_WITHOUT_ROUTING`) requests processing without onward routing or delivery. Where a header `#BAR#` note and an explicit `puf:ComplianceActivatedIndicator` are both present, the explicit indicator takes precedence.
